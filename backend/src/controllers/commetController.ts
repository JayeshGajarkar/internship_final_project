...
export class CommentController {
  static async addComment(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const comment=await commentService.addComment(taskId, userId, commentDTO);
  ...
  static async addFile(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const comment=await commentService.addFile(taskId, userId, file);
      res.status(200).json({ message: "File uploaded successfully",comment:comment});
    } catch (error) {
      console.log(error);
      next(new AppError(error.message, 500));
    }
