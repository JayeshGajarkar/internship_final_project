...
export class ProjectController {
    static async addProject(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            const project=await ProjectService.addProject(projectDTO, userId);
            res.status(201).json({ message: "Project added successfully",project:project});
        } catch (error) {
            console.log(error);
    ...
    static async updateProject(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            const userId = parseInt(req.body.userId);
            const projectId = parseInt(req.params.id);
            const project=await ProjectService.updateProject(projectId, userId, projectDTO);
